...
public class SiswaDAO {
    ...
    public boolean insert(Siswa siswa) {
        ...
        try {
            ...
            ps.setDouble(7, siswa.getNilaiUjian());
    ...
    public List<Siswa> getAll() {
        ...
        String sql = "SELECT s.*, k.nama_kelas FROM siswa s " +
                     ...
                     "ORDER BY s.id_kelas ASC, s.ranking_kelas ASC";
        try {
    ...
    public List<Siswa> getByKelas(int idKelas) {
        ...
        String sql = "SELECT s.*, k.nama_kelas FROM siswa s " +
                     "JOIN kelas k ON s.id_kelas = k.id_kelas " +
                     "WHERE s.id_kelas=? ORDER BY ranking_kelas ASC";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, idKelas);
    ...
    public boolean update(Siswa siswa) {
        String sql = "UPDATE siswa SET nama_siswa=?, jenis_kelamin=?, alamat=?, " +
                     "id_kelas=?, nilai_tugas=?, nilai_ujian=?, nilai_akhir=?, " +
                     "grade=?, ranking_kelas=? WHERE id_siswa=?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ...
            ps.setDouble(6, siswa.getNilaiUjian());
            ps.setDouble(7, siswa.getNilaiAkhir());
            ps.setString(8, siswa.getGrade());
            ps.setInt(9, siswa.getRanking());
            ps.setString(10, siswa.getIdSiswa());
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
    ...
    public List<Siswa> search(String keyword) {
        ...
        String sql = "SELECT s.*, k.nama_kelas FROM siswa s " +
                     "JOIN kelas k ON s.id_kelas = k.id_kelas " +
                     "WHERE s.nama_siswa LIKE ? OR s.id_siswa LIKE ? " +
    ...
    public boolean isIdExist(String idSiswa) {
        String sql = "SELECT id_siswa FROM siswa WHERE id_siswa=?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, idSiswa);
            ResultSet rs = ps.executeQuery();
    ...
    private Siswa mapRow(ResultSet rs) throws SQLException {
        ...
        s.setNilaiUjian(rs.getDouble("nilai_ujian"));
